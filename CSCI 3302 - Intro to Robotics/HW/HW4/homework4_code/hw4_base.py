...
def expand_nr(img_mask, cur_coord, coordinates_in_blob):
  ...
  while len(coordinate_list) > 0:
    ...
    #cur_coordinate[0] < 0 || cur_corrdinate[1] < 0 || img_mask.shape[0] <= cur_coordinate[0] || img_mask.shape[1] <= cur_coordinate[1]
    if (cur_coordinate[0] < 0) or (cur_coordinate[1] < 0) or (img_mask.shape[0] <= cur_coordinate[0]) or (img_mask.shape[0] <= cur_coordinate[1]):
        continue
...
def get_blobs(img_mask):
  ...
  for y in range(0, img_mask_height):
      for x in range(0, img_mask_width):
          if (local_mask[y, x] == 1):
              blobs_coord = []
              blobs_coords = expand_nr(local_mask, (y, x), blobs_coord)
...
def get_blob_centroids(blobs_list):
  ...
  for i in blobs_list:
      ...
      blob_centroid = np.mean(i, axis = 0)
      object_positions_list.append(blob_centroid)
...
def main():
  ...
  add_color_range_to_detect([0, 1, 112], [60, 75, 243]) # Detect darkest red to lightest red
  add_color_range_to_detect([20, 83, 0], [135, 240, 110]) # Detect darkest green to lightest green
  add_color_range_to_detect([201, 77, 12], [232, 133, 79]) # Detect darkest blue to lightest blue
  add_color_range_to_detect([0, 170, 221], [44, 243, 255]) # Detect darkest yellow to lightest yellow
