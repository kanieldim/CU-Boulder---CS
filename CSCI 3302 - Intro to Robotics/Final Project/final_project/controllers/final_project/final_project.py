...
NUM_CELLS1 = 20
RESOLUTION1 = 1.0 / NUM_CELLS1
world_map1 = [[0 for x in range(NUM_CELLS1)] for y in range(NUM_CELLS1)]
path1 = []
...
def update_map(lidar_readings_array):
    ...
    for counter, distance in enumerate(lidar_readings_array):
        ...
        if (distance > 0.0 and distance <= 1.414):
            ...
            world_coord1 = convert_lidar_reading_to_world_coord(counter, distance)
            map_coord1 = transform_world_coord_to_map_coord(world_coord1)
            if not map_coord1: continue

            i ,j = map_coord1
            #print(i,j)
            world_map1[19-i][19-j] = 1
            
def display_map1(m,path1):
    """
    @param m: The world map matrix to visualize
    """
    nrow, ncol = m.shape
    ...
    # create colormap
    cmap = colors.ListedColormap(['slategray','blue'])
    ...
    if path1:
        for p in range(len(path1)-1):
            point = path1[p]
            nextpoint = path1[p+1]
            plt.plot([point[0],nextpoint[0]], [point[1],nextpoint[1]], c='white')
        
    #plt.show()
    plt.show(block=False)
    plt.pause(0.01) # 3 seconds, I use 1 usually
    plt.close("all")
...
def main():
    ...
    while robot.step(SIM_TIMESTEP) != -1:
        ...
        robot_x,robot_y = transform_world_coord_to_map_coord((pose_x, pose_y))
        
        
        #path1.append((robot_x,19-robot_y))
        update_map(lidarValues)

        np_map = np.array(world_map1)
        display_map1(np_map,path1)
