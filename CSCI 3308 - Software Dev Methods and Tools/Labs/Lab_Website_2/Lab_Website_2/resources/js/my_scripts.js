...
function viewStudentStats(name, toggle)
{
...
function loadStatsPage()
{
	...
	for(row_counter = 2; row_counter < table.rows.length; row_counter++)
	{
		...
		homeScore = table.rows[row_counter].cells[2].innerHTML;//Read in a cells current value
		awayScore = table.rows[row_counter].cells[3].innerHTML;//Read in a cells current value

		if( parseInt(homeScore) > parseInt(awayScore) )
		{
			table.rows[row_counter].cells[4].innerHTML = home;//Update the actual html of the cell
			numOfWins++;
		}
		else
		{
...
{
	var list = "";
	var dropDownMenu = document.getElementById("player_selector");
	for( var i = 0; i< players.length; i++)
	{
	list = list + '<p><a href = # onclick = switchPlayers(' + i + ')>' + players[i].name + '</a><p>'
...
function switchPlayers(playerNum)
{
	...
	var AvgPassYards = document.getElementById("p_yards").innerHTML / document.getElementById("g_played").innerHTML;
	var AvgRushYards = document.getElementById("r_yards").innerHTML / document.getElementById("g_played").innerHTML;
	var AvgReceiveYards = document.getElementById("rec_yards").innerHTML / document.getElementById("g_played").innerHTML;
	document.getElementById("avg_p_yards").innerHTML = AvgPassYards.toFixed(0);
	document.getElementById("avg_r_yards").innerHTML = AvgRushYards.toFixed(0);
	document.getElementById("avg_rec_yards").innerHTML = AvgReceiveYards.toFixed(0);
